...
import { 
  analyzePhoneNumber, 
  analyzeEmailAddress, 
  POPULAR_OSINT_PLATFORMS, 
  generateCaseDorks 
} from '../services/osintToolsService';

export default function OsintLab({ activeCase, onDiscoverClue }) {
  const [subTab, setSubTab] = useState('phone_recon'); // 'phone_recon', 'email_recon', 'username_recon', 'case_dorks', 'case_intel'
  ...
  const [emailInput, setEmailInput] = useState('investigator.target@gmail.com');
  const [emailResult, setEmailResult] = useState(() => analyzeEmailAddress('investigator.target@gmail.com'));
  ...
  const [terminalLogs, setTerminalLogs] = useState([
    "[OSINT-SYSTEM] Initialized Multi-Vector Intelligence Engine v5.2",
    "[INDIA-DOT] Loaded National Numbering Plan & Telecom MSC Series",
    "[READY] Search by Indian Mobile (+91), Email, or Username Handle."
  ]);
  ...
  const handlePhoneAnalyze = (customNum = null) => {
    ...
    setTerminalLogs(prev => [
      ...
      `[NAME-DISCOVERY] 5 Reverse Lookup methods & UPI VPA permutations ready.`
    ]);
  };

  const handleEmailAnalyze = (customEmail = null) => {
    const mail = customEmail || emailInput;
    if (!mail.trim() || !mail.includes('@')) return;
    soundService.playTypewriter();
    const result = analyzeEmailAddress(mail);
    setEmailResult(result);
    setTerminalLogs(prev => [
      ...prev,
      `> ANALYZING EMAIL: ${mail}`,
      `[DOMAIN] ${result.domain} (${result.providerType})`,
      `[BREACH-INTEL] Epieos Google Profiler & Breach search links generated.`
    ]);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 h-[calc(100vh-4.5rem)] flex flex-col gap-4 font-mono">
      ...
      <div className="glass-panel p-3 rounded-2xl border border-noir-800 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-noir-400">
              Multi-vector investigative search by Phone Number, Email Address, Username Handle, and Legal Dorks.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1 bg-noir-900 p-1 rounded-xl border border-noir-800 overflow-x-auto no-scrollbar">
          {[
            ...
            { id: 'email_recon', label: '📧 Email Recon', icon: Mail },
            { id: 'username_recon', label: '👤 Username Recon', icon: UserCheck },
            { id: 'case_dorks', label: '⚖️ Legal & FIR Dorks', icon: FileText },
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 overflow-hidden">
        ...
        <div className="lg:col-span-2 glass-panel p-4 rounded-2xl border border-noir-800 flex flex-col overflow-hidden">
          ...
          {/* TAB 1: REAL INDIAN & GLOBAL MOBILE NUMBER OSINT (WITH REVERSE PHONE-TO-NAME) */}
          {subTab === 'phone_recon' && (
            <div className="flex-1 flex flex-col overflow-y-auto space-y-4 pr-1">
              ...
              {phoneResult && (
                <div className="space-y-3">
                  ...
                  {/* 🔍 REVERSE PHONE-TO-NAME INVESTIGATIVE METHODS */}
                  <div className="bg-noir-950/90 p-4 rounded-xl border border-amber-600/50 shadow-lg">
                    <div className="flex items-center space-x-2 mb-2 pb-2 border-b border-amber-800/60">
                      <User className="w-4 h-4 text-amber-400" />
                      <h4 className="text-xs font-bold text-amber-300 uppercase">
                        How to Discover the Real Name Behind This Number:
                      </h4>
                    </div>

                    <p className="text-[11px] text-noir-300 mb-3 leading-relaxed">
                      Telecom carriers in India and worldwide do not publish subscriber names publicly due to TRAI privacy regulations. Use these <strong>5 proven open-source forensic methods</strong> to identify the registered name:
                    </p>

                    <div className="space-y-2.5">
                      {phoneResult.nameDiscoveryMethods.map((m, idx) => (
                        <div key={idx} className="p-3 rounded-lg bg-noir-900 border border-noir-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className="flex-1">
                            <span className="text-xs font-bold text-white block">{m.method}</span>
                            <p className="text-[10px] text-noir-400 mt-0.5">{m.description}</p>
                          </div>
                          {m.url && (
                            <a
                              href={m.url}
                              target="_blank"
                              rel="noreferrer"
                              className="px-3 py-1.5 rounded-lg bg-noir-800 hover:bg-cyber-cyan hover:text-noir-950 border border-noir-700 text-xs font-bold text-cyber-cyan flex items-center space-x-1.5 transition-all whitespace-nowrap flex-shrink-0"
                            >
                  ...
                  {phoneResult.possibleUPIVPAs.length > 0 && (
                    <div className="bg-noir-950/90 p-4 rounded-xl border border-noir-800">
                      <h4 className="text-xs font-bold text-emerald-400 uppercase mb-1.5 flex items-center space-x-1.5">
                        <CreditCard className="w-4 h-4" />
                        <span>UPI Banking Handles (Query in GPay / PhonePe / Paytm to reveal legal bank name)</span>
                      </h4>
                  ...
                  <div className="bg-noir-950/90 p-4 rounded-xl border border-noir-800">
                    <h4 className="text-xs font-bold text-cyber-cyan uppercase mb-2">
                      Target Google Dorks for Public Mentions & Leaked Invoices
                    </h4>
          ...
          {/* TAB 2: EMAIL OSINT & BREACH RECON */}
          {subTab === 'email_recon' && (
            <div className="flex-1 flex flex-col overflow-y-auto space-y-4 pr-1">
              ...
              <div className="bg-noir-950/80 p-4 rounded-xl border border-noir-800">
                <label className="block text-xs font-bold text-cyber-cyan uppercase mb-2">
                  Enter Target Email Address for Reverse Lookup & Breach Profiling:
                </label>
                <div className="flex items-center space-x-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. suspect@company.com or john.doe@gmail.com"
                      className="w-full bg-noir-900 text-xs text-white pl-9 pr-4 py-2.5 rounded-xl border border-noir-700 focus:outline-none focus:border-cyber-cyan font-mono"
                    />
                  </div>
                  <button
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-noir-950/90 p-4 rounded-xl border border-noir-800">
                    <h4 className="text-xs font-bold text-white uppercase mb-3 flex items-center space-x-1.5">
                      ...
                      <span>Live Reverse Email & Breach Databases</span>
                    </h4>

                    <div className="space-y-2">
                      {emailResult.investigationLinks.map((link, idx) => (
                        <div key={idx} className="p-3 rounded-lg bg-noir-900 border border-noir-800 flex items-center justify-between gap-2">
                          <div>
                            ...
                            <p className="text-[10px] text-noir-400 mt-0.5">{link.description}</p>
                          </div>
                          <a
                            href={link.url}
                            target="_blank"
          ...
          {/* TAB 3: USERNAME RECON */}
          {subTab === 'username_recon' && (
            <div className="flex-1 flex flex-col overflow-y-auto space-y-4 pr-1">
              <div className="bg-noir-950/80 p-4 rounded-xl border border-noir-800">
                <label className="block text-xs font-bold text-cyber-cyan uppercase mb-2">
                  Target Username / Handle Multi-Network Search:
                </label>
                <div className="flex items-center space-x-2">
                  <div className="relative flex-1">
                    ...
                    >
                      <div>
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span className="text-[8px] uppercase px-1 py-0.2 rounded bg-noir-900 text-noir-400 border border-noir-800">
                            {platform.category}
                          </span>
                        </div>
                        <span className="text-[10px] text-noir-400 truncate block max-w-[150px] mt-0.5">
                          @{usernameQuery}
                        </span>
                      </div>
          ...
          {/* TAB 4: LEGAL & POLICE FIR GOOGLE DORKS */}
          {subTab === 'case_dorks' && (
          ...
          {/* TAB 5: CASE DIGITAL LOGS & FORENSICS */}
          {subTab === 'case_intel' && (
            <div className="flex-1 flex flex-col overflow-y-auto space-y-3 pr-1">
        ...
        <div className="glass-panel p-4 rounded-2xl border border-noir-800 flex flex-col overflow-hidden bg-noir-950">
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5 text-[11px] text-noir-300 font-mono pr-1 bg-black/50 p-3 rounded-xl border border-white/5">
            {terminalLogs.map((log, index) => (
              <div 
                key={index} 
                className={`${log.startsWith('>') ? 'text-amber-400 font-bold' : log.includes('[CARRIER]') || log.includes('[BREACH-INTEL]') ? 'text-cyber-cyan font-bold' : 'text-noir-400'}`}
              >
                {log}
              </div>
            ))}
          ...
          <div className="mt-3 p-2.5 rounded-xl bg-noir-900 border border-noir-800 text-[10px] text-noir-400 flex items-center justify-between">
            <span>Vectors: Phone • Email • Handle</span>
